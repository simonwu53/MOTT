...
ex.add_named_config('mot17_half', 'cfgs/train_mot17_half.yaml')
...
ex.add_named_config('coco_person_masks_mott', 'cfgs/train_coco_person_masks_mott.yaml')
ex.add_named_config('model_trackformer', 'cfgs/train_model_trackformer.yaml')
ex.add_named_config('model_motr', 'cfgs/train_model_motr.yaml')
...
def train(args: Namespace) -> None:
    ...
    lr_drop = [args.lr_drop] if isinstance(args.lr_drop, int) else args.lr_drop
        ...
        if args.masks and args.load_mask_head_from_model is not None:
            ...
            for k, v in resume_state_dict.items():
                ...
                if 'bbox_attention' in k or 'mask_head' in k:
                    v_ckpt = checkpoint_mask_head['model'][k]
                    if v.shape == v_ckpt.shape:
                        print(f'Load {k} {tuple(v.shape)} from mask head model.')
                        resume_state_dict[k] = v_ckpt
                    else:
                        # adapt segmentation state dict for MOTT (256 -> 288)
                        if args.hidden_dim == 288:
                            if 'linear.weight' in k:
                                # (256, 256) -> (288, 288)
                                v.view(1024, 9, 9)[:, :8, :8] = v_ckpt.view(1024, 8, 8)
                                v.view(1024, 9, 9)[:, 8, 8] = v_ckpt.view(1024, 8, 8).mean((1, 2))
                                v.view(1024, 9, 9)[:, :8, 8] = v_ckpt.view(1024, 8, 8).mean(dim=2)
                                v.view(1024, 9, 9)[:, 8, :8] = v_ckpt.view(1024, 8, 8).mean(dim=1)
                            elif 'linear.bias' in k:
                                # (256,) -> (288,)
                                v.view(32, 9)[:, :8] = v_ckpt.view(32, 8)
                                v.view(32, 9)[:, 8] = v_ckpt.view(32, 8).mean(dim=1)
                            elif 'gn1' in k or 'lay1.bias' in k:
                                # (264,) -> (296,)
                                v_temp = torch.empty(288)
                                ckpt_temp = torch.empty(256).copy_(v_ckpt[:256])
                                v_temp.view(32, 9)[:, :8] = ckpt_temp.view(32, 8)
                                v_temp.view(32, 9)[:, 8] = ckpt_temp.view(32, 8).mean(dim=1)
                                v[:288] = v_temp
                                v[-8:] = v_ckpt[-8:]
                            elif 'lay1.weight' in k:
                                # (264, 264, 3, 3) -> (296, 296, 3, 3)
                                v_temp = torch.empty(288, 288, 3, 3)
                                ckpt_temp = torch.empty(256, 256, 3, 3).copy_(v_ckpt[:256, :256])
                                v_temp.view(9216, 9, 9)[:, :8, :8] = ckpt_temp.view(9216, 8, 8)
                                v_temp.view(9216, 9, 9)[:, 8, 8] = ckpt_temp.view(9216, 8, 8).mean((1, 2))
                                v_temp.view(9216, 9, 9)[:, :8, 8] = ckpt_temp.view(9216, 8, 8).mean(dim=2)
                                v_temp.view(9216, 9, 9)[:, 8, :8] = ckpt_temp.view(9216, 8, 8).mean(dim=1)
                                v[:288, :288] = v_temp
                                v[-8:, -8:] = v_ckpt[-8:, -8:]
                            elif 'lay2.weight' in k:
                                # (128, 264, 3, 3) -> (128, 296, 3, 3)
                                v_temp = torch.empty(128, 288, 3, 3)
                                ckpt_temp = torch.empty(128, 256, 3, 3).copy_(v_ckpt[:, :256])
                                v_temp.view(36864, 9)[:, :8] = ckpt_temp.view(36864, 8)
                                v_temp.view(36864, 9)[:, 8] = ckpt_temp.view(36864, 8).mean(dim=1)
                                v[:, :288] = v_temp
                                v[:, -8:] = v_ckpt[:, -8:]
                            elif 'adapter1.weight' in k or 'adapter2.weight' in k or 'adapter3.weight' in k:
                                # (128, N, 1, 1) -> (128, N//4, 4, 1, 1) -> (128, N//4, 1, 1)
                                ckpt_shape = tuple(v_ckpt.shape)
                                v = v_ckpt.view(ckpt_shape[0], ckpt_shape[1] // 4, 4, ckpt_shape[2], ckpt_shape[3]).mean(dim=2)
                            else:
                                raise NotImplementedError(f"No rule for {k} with shape {v.shape}.")

                            print(f'Load {k} {tuple(v.shape)} from mask head model. (Adapted from {tuple(v_ckpt.shape)})')
                            resume_state_dict[k] = v
