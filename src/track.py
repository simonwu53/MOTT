...
# * Added: Supress user warnings
# * Added: Reading experimental config
...
ex.add_named_config('exp', 'cfgs/track_exp.yaml')
...
def main(seed, dataset_name, obj_detect_checkpoint_file, tracker_cfg,
    ...
    # object detection
    if obj_detector_model is None:
        obj_detector, obj_detector_post, img_transform = \
            initialize_model(obj_detect_checkpoint_file, _logger=_log, _cuda=True, ckpt_version=3, _tracking=False)
    else:
    ...
    if obj_detector_model is None:
        ...
        if output_dir is not None:
            _log.info(f"WRITE SUMMARY")
            summary.to_csv(os.path.join(output_dir, 'summary.txt'), sep=',', index=False)
