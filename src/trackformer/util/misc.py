...
# new method to check torchvision version numbers to support 0.1x.x version numbers
major_version, minor_version = torchvision.__version__.split('.')[:2]
if float(major_version) < 1 and float(minor_version) < 5:
    import math
    from torchvision.ops.misc import _NewEmptyTensorOp
    _new_empty_tensor = _NewEmptyTensorOp.apply
...
def interpolate(input, size=None, scale_factor=None, mode="nearest", align_corners=None):
    ...
    # new method to check torchvision version numbers to support 0.1x.x version numbers
    major_version, minor_version = torchvision.__version__.split('.')[:2]
    if float(major_version) < 1 and float(minor_version) < 7:
        if input.numel() > 0:
